...
- (instancetype)initWithAnimatedGIFData:(NSData *)data imageDrawingBlock:(UIImage *(^)(UIImage *))imageDrawingBlock
{
    ...
    if (self) {
        ...
        CGFloat allFramesDataSizeLimit = (CGFloat)FLAnimatedImageDataSizeCategoryAll;
        CGFloat defaultCacheDataSizeLimit = (CGFloat)FLAnimatedImageDataSizeCategoryDefault;
        if (animatedImageDataSize <= allFramesDataSizeLimit) {
            _frameCacheSizeOptimal = self.frameCount;
        } else if (animatedImageDataSize <= defaultCacheDataSizeLimit) {
